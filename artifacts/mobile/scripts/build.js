...
const METRO_PORT = process.env.METRO_PORT ? parseInt(process.env.METRO_PORT, 10) : 8081;

async function checkMetroHealth(port = METRO_PORT) {
  try {
...
let activeMetroPort = METRO_PORT;

async function startMetro(expoPublicDomain, expoPublicReplId) {
  // If METRO_PORT is already running (e.g. a reuse from previous build), skip spawning
  const isRunning = await checkMetroHealth(METRO_PORT);
  if (isRunning) {
    console.log(`Metro already running on port ${METRO_PORT}`);
    activeMetroPort = METRO_PORT;
    return;
  }

  // Detect port conflict and pick alternative
  const busy = await isPortBusy(METRO_PORT);
  const port = busy ? (METRO_PORT === 8081 ? 8082 : METRO_PORT + 1) : METRO_PORT;
  if (busy) console.log(`Port ${METRO_PORT} busy — using ${port} instead`);
  activeMetroPort = port;
  ...
  for (let i = 0; i < 60; i++) {
    ...
    const healthy = await checkMetroHealth(activeMetroPort);
    if (healthy) {
      console.log(`Metro ready on port ${activeMetroPort}`);
      return;
    }
  }
...
async function downloadBundle(platform, timestamp) {
  ...
  const url = new URL(`http://localhost:${activeMetroPort}/${bundlePath}.bundle`);
...
async function downloadManifest(platform) {
  ...
  try {
    ...
    const response = await fetch(`http://localhost:${activeMetroPort}/manifest`, {
...
function extractAssets(timestamp) {
  ...
  const extractFromBundle = (bundle, platform) => {
    for (const match of bundle.matchAll(assetPattern)) {
      ...
      const tempUrl = new URL(`http://localhost:${activeMetroPort}${originalPath}`);
...
async function downloadAssets(assets, timestamp) {
  ...
  const downloadPromises = assets.map(async (asset) => {
    const tempUrl = new URL(`http://localhost:${activeMetroPort}${asset.originalPath}`);
    const unstablePath = tempUrl.searchParams.get("unstable_path");
...
function updateBundleUrls(timestamp, baseUrl) {
  const updateForPlatform = (platform) => {
    ...
    bundle = bundle.replace(
      ...
      (_match, capturedPath) => {
        const tempUrl = new URL(`http://localhost:${activeMetroPort}${capturedPath}`);
        const unstablePath = tempUrl.searchParams.get("unstable_path");
