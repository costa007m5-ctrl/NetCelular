...
async function getSignedUrl(key: string): Promise<string> {
  const data = await apiFetch<{ url: string }>(`/signed-url?key=${encodeURIComponent(key)}`);
  return data.url;
}
    ...
    <View style={{ flex: 1 }}>
      ...
      {error ? (
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={[styles.center, { paddingTop: 60 }]}>
              <Text style={styles.errorText}>Nenhum título encontrado</Text>
            </View>
          }
...
export default function AdminScreen() {
  ...
  const insets = useSafeAreaInsets();
  const { user } = useAuth();
  const [view, setView] = useState<AdminView>({ screen: "catalog" });
  ...
  return (
    <View style={[{ flex: 1, backgroundColor: colors.background }]}>
      ...
      {/* Header — shown only on catalog */}
      {view.screen === "catalog" && (
        <View style={[styles.header, { paddingTop: insets.top + 12 }]}>
          <View>
            <Text style={styles.headerTitle}>Admin</Text>
            <Text style={styles.headerSub}>Cloudflare R2 · Catálogo</Text>
          </View>
        </View>
      )}
      ...
      {view.screen === "catalog" && (
        <CatalogGrid
          onSelect={(entry) => setView({ screen: "seasons", entry })}
        />
      )}
