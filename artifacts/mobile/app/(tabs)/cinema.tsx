...
async function fetchCinema(attempt = 0): Promise<CinemaData> {
  try {
    ...
    if (res.warming && attempt < 5) {
      // Server still warming — retry every 5s for up to ~25s
      await new Promise((resolve) => setTimeout(resolve, 5000));
      return fetchCinema(attempt + 1);
    }
    // If warmed but still empty (0 results), retry a couple more times
    if (res.ok && res.total === 0 && attempt < 2) {
      await new Promise((resolve) => setTimeout(resolve, 5000));
      return fetchCinema(attempt + 1);
    }
    return res;
